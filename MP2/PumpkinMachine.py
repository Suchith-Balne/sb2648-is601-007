...
class PumpkinMachine:
    ...
    def print_current_pumpkin(self):
        ...
        # UCID: sb2648 Date: 10/23/2023
        # This method takes care of cost calculation for the products present in inprogress_pumkin array depending on the category and returns the total cost of all the products.
    def calculate_cost(self):
        total_cost = 0
        for pumpkin in self.inprogress_pumpkin:
            total_cost += pumpkin.cost
        return total_cost
    ...
    def run(self):
        ...
        #UCID: sb2648 Date:10/23/2023
        # OutOfStockException is handled here, and a message to the user is displayed showing a valid out of stock message in particular they are trying to add category.
        except OutOfStockException:
            print(f"Out of stock in {self.currently_selecting.name} category.")
        
        #UCID: sb2648 Date: 10/23/2023
        # Whenever the array is filled with items, it needs to be cleaned in order to accomodate the products for the next customers. This is handled here and cleaning can be performed by giving a "clean" keyword and prints the message after cleaning.
        except NeedsCleaningException:
            clean_input = input("The machine needs cleaning. Type 'clean' to clean the machine:")
            if clean_input.strip().lower() == "clean":
                self.clean_machine()
                print("Machine has been cleaned.")

        # UCID: sb2648 Date:10/23/23
        # This exception is raised whenever user tries to enter a invalid item which is not present in the pumkins list. Also, message is displayed in which category this exception is occured.
        except InvalidChoiceException:
            print(f"Invalid choice in {self.currently_selecting.name} category.")

        #  UCID: sb2648 Date:10/23/23
        # This exception is raised when user tries to add products more than the maximum allowed range defined for the each category and we make user to follow selection for next categories by changing the current selection to next category.
        except ExceededRemainingChoicesException:
            print(f"Exceeded remaining choices in {self.currently_selecting.name} category. Moving to the next category.")
            if self.currently_selecting != STAGE.Extra:
                self.currently_selecting = STAGE.Extra
            else:
                self.currently_selecting = STAGE.Pay
        ...
        #  UCID: sb2648 Date:10/23/23
        # This exception is raised when a user tries to enter the amount other than the displayed total and an appropriate message is shown to the user to enter total amount to complete payment.
        except InvalidPaymentException:
            print("Invalid payment amount. Please pay the total amount.")
